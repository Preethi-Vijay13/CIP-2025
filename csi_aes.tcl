...
set SECRET_KEY "mysecretkey12345"  ;# 16-byte key for AES-128

proc encrypt_csi {csi key} {
    set csi_int [expr int($csi * 1000)]  ;# Convert CSI to integer
    set csi_bytes [binary format I $csi_int]  ;# Convert to 4-byte binary
    set encrypted_bytes [aes::aes -mode cbc -dir encrypt -key $key $csi_bytes]
    binary scan $encrypted_bytes H* encrypted_hex  ;# Convert to hex
    return $encrypted_hex
}

proc decrypt_csi {encrypted_hex key} {
    set encrypted_bytes [binary format H* $encrypted_hex]  ;# Convert hex to binary
    set decrypted_bytes [aes::aes -mode cbc -dir decrypt -key $key $encrypted_bytes]
    binary scan $decrypted_bytes I csi_int  ;# Convert to integer
    return [expr $csi_int / 1000.0]  ;# Convert back to float
}

# Hash function to generate node authentication token
proc generate_hash {node_id encrypted_csi key} {
    return [sha2::sha256 -hex "$node_id-$encrypted_csi-$key"]
}
