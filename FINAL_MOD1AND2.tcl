...
proc activate_jamming {jam eav ch} {
    ...
    # Create a Null agent at the eavesdropper to receive jamming packets
    set sink [new Agent/Null]
    $ns attach-agent $eav $sink

    # Connect the jammer to the eavesdropper via the Cluster Head
    $ns connect $udp $sink

    # Configure jamming traffic
    $cbr set packetSize_ 1024  ;# Larger packet size for high interference
    $cbr set interval_ 0.005   ;# Higher transmission rate
    $cbr set fid_ 3            ;# Unique flow ID for jamming traffic
    $ns color 3 Purple         ;# Set color for jamming packets

    # Start jamming at 1.5s
    $ns at 1.5 "$cbr start"

    # Stop jamming at 2.0s
    $ns at 2.0 "$cbr stop"
    $ns at 2.0 "$jam color $node_color($jam)"
    $ns at 2.0 "$eav color $Eavesdropper_color"

    puts "Jamming activated for $jam targeting $eav via $ch at 1.5s"
